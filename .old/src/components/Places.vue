              ...
              <div class="header-container">
                ...
                <div class="yelp">
                  <div>
                    <img
                      :src="
                        '../src/assets/images/yelp/' + place.rating + '.png'
                      "
                      class="card-rating"
                    />
                    <span class="review-count"
                      >Based off of {{ place.review_count }} reviews</span
                    >
                  </div>
                  <a :href="place.url" target="_blank"
                    ><img
                      :src="'../src/assets/images/yelp/logo.png'"
                      class="yelp-logo"
                  /></a>
                </div>
              </div>
            ...
            <div class="card-content">
              <div class="media">
                <div class="media-content">
                  Tags:
                  <span v-if="place.categories[0]" class="tag">{{
                    place.categories[0].title
                  }}</span>
                  <span v-if="place.categories[1]" class="tag">{{
...
export default {
  ...
  mounted() {
    eventBus.$on("newLocation", location => {
      ...
      const yelp_token = null;
      const proxy = "https://cors-anywhere.herokuapp.com/";

      axios
        .get(
          `${proxy}${yelp_url}?&latitude=${location.lat}&longitude=${location.lng}&radius=5000&term=${this.locationType}`,
          {
            headers: {
